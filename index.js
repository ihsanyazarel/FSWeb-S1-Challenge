...
// console.log(`${3}. indekste bulunan fenomen: ${fenomenler[3].profile}`);
...
function fenomenEkle(array,siraNo, profil, takipciSayisi, gonderiSayisi, platformAdi) {
  let newArray = array.slice();
  let newObject = {
    "number": siraNo,
    "profile": profil,
...
function enFenomenler(array) {
  ...
  array.forEach(element => {
    if(element.followers>100000000){
      newArray.push(element.profile)
    }
  });
  return newArray;
}
...
function platformaGoreCokGonderiYapanFenomen(array, platformAdi){
  let newArray = new Array();
  ...
  newArray.forEach(element => {
    if(element.posts>quantity && element.posts !="NA"){
      quantity = element.posts;
      newObject = element;
    }
  });
    return newObject.profile;
}
